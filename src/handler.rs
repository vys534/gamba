...
impl EventHandler for Handler {
    async fn message(&self, ctx: Context, m: Message) {
        ...
        match crate::db::on_message::handle(&m.author, self.redis.clone(), &mut db_conn).await {
            Ok(_) => (),
            Err(e) => {
                handle_error(ctx, m, e, false).await;
                return;
        ...
        for c_test in crate::model::command::command_list().await {
            if cmd_name == c_test.name || cmd_name == c_test.shorthand {
                let res = c_test.inner.exec(&ctx, &m, db_conn, redis_conn, args).await;
                match res {
                    Ok(_) => (),
                    Err(e) => handle_error(ctx, m, e, true).await,
                }
                break;
            }
        }
